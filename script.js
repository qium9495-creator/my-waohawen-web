...
function applyLanguage(lang){const nextLang=I18N[lang]?lang:'en';document.documentElement.lang=nextLang;document.documentElement.dir=nextLang==='ar'?'rtl':'ltr';if(currentLanguage)currentLanguage.textContent=I18N[nextLang]?.name||'English';document.querySelectorAll('[data-i18n]').forEach(el=>el.textContent=tx(nextLang,el.dataset.i18n));document.querySelectorAll('[data-i18n-html]').forEach(el=>el.innerHTML=tx(nextLang,el.dataset.i18nHtml));document.querySelectorAll('[data-i18n-placeholder]').forEach(el=>el.placeholder=tx(nextLang,el.dataset.i18nPlaceholder));document.querySelectorAll('[data-i18n-alt]').forEach(el=>el.alt=tx(nextLang,el.dataset.i18nAlt));document.querySelectorAll('[data-i18n-aria-label]').forEach(el=>el.setAttribute('aria-label',tx(nextLang,el.dataset.i18nAriaLabel)));saveLanguage(nextLang);if(page==='products'){refreshSecondaryLabels(nextLang);refreshProductCount(nextLang)}}
 ...
 if(wall){
  ...
  const renderWall=series=>{
   ...
   for(let rowIndex=0;rowIndex<3;rowIndex++){
    ...
    sequence.forEach(item=>{const link=document.createElement('a');link.className='drift-wall__tile';link.href=item.href;link.innerHTML=`<span class="drift-wall__inner"><img src="${item.image}" alt="" loading="lazy" decoding="async"/><span class="drift-wall__shade"></span><span class="drift-wall__caption"><strong data-i18n="${item.title}">${tx(document.documentElement.lang||'en',item.title)}</strong></span></span>`;track.appendChild(link)});
...
if(page==='detail'){
 ...
 item.gallery.slice(0,5).forEach((src,index)=>{const isVideo=/\.(mp4|webm|ogg)$/i.test(src),button=document.createElement('button');button.type='button';button.className='detail-thumbnail'+(index===0?' active':'');button.setAttribute('aria-label',isVideo?`播放产品视频 ${index+1}`:`查看第 ${index+1} 张产品图`);button.innerHTML=isVideo?'<span class="detail-video-mark">▶<small>VIDEO</small></span>':`<img src="${src}" alt="" />`;button.addEventListener('click',()=>{showMedia(src);thumbs.querySelectorAll('.detail-thumbnail').forEach(node=>node.classList.remove('active'));button.classList.add('active')});thumbs.appendChild(button)});
