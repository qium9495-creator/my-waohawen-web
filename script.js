...
mainNav?.querySelectorAll('.nav-item').forEach((item,index)=>{
 ...
 const panel=item.querySelector(':scope > .home-mega');
 if(!trigger||!panel)return;
 if(!panel.id)panel.id=`mobileNavPanel${index+1}`;
 const toggle=document.createElement('button');
 toggle.type='button';toggle.className='mobile-submenu-toggle';toggle.setAttribute('aria-controls',panel.id);toggle.setAttribute('aria-expanded','false');
 const setExpanded=expanded=>{item.classList.toggle('expanded',expanded);trigger.setAttribute('aria-expanded',String(expanded));toggle.setAttribute('aria-expanded',String(expanded));toggle.setAttribute('aria-label',`${expanded?'Close':'Open'} ${trigger.textContent.trim()} submenu`)};
 const toggleExpanded=()=>{
  const shouldExpand=!item.classList.contains('expanded');
  mainNav.querySelectorAll('.nav-item.expanded').forEach(openItem=>{openItem.classList.remove('expanded');openItem.querySelector(':scope > a')?.setAttribute('aria-expanded','false');openItem.querySelector(':scope > .mobile-submenu-toggle')?.setAttribute('aria-expanded','false')});
  setExpanded(shouldExpand);
  if(shouldExpand)mainNav.scrollTo({top:Math.max(0,item.offsetTop-2),behavior:'smooth'});
 };
 trigger.setAttribute('aria-haspopup','true');trigger.setAttribute('aria-expanded','false');setExpanded(false);
 trigger.addEventListener('click',event=>{if(matchMedia('(max-width: 1114px)').matches){event.preventDefault();toggleExpanded()}});
 toggle.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();toggleExpanded()});
 trigger.after(toggle);
 item.addEventListener('keydown',event=>{if(event.key==='Escape'){setExpanded(false);trigger.focus()}});
});
...
const desktopNavCanHover=()=>matchMedia('(min-width: 1115px) and (hover: hover) and (pointer: fine)').matches;
const clearDesktopNavOpen=()=>mainNav?.querySelectorAll('.home-nav-item.desktop-open').forEach(item=>{item.classList.remove('desktop-open');item.querySelector(':scope > a')?.setAttribute('aria-expanded','false')});
mainNav?.querySelectorAll('.home-nav-item').forEach(item=>{
 const trigger=item.querySelector(':scope > a'),panel=item.querySelector(':scope > .home-mega');
 if(!trigger||!panel)return;
 let closeTimer=0;
 const openDesktop=()=>{if(!desktopNavCanHover())return;clearTimeout(closeTimer);mainNav.querySelectorAll('.home-nav-item.desktop-open').forEach(openItem=>{if(openItem!==item){openItem.classList.remove('desktop-open');openItem.querySelector(':scope > a')?.setAttribute('aria-expanded','false')}});item.classList.add('desktop-open');trigger.setAttribute('aria-expanded','true')};
 const closeDesktop=delay=>{if(!desktopNavCanHover())return;clearTimeout(closeTimer);closeTimer=setTimeout(()=>{if(item.matches(':hover')||item.contains(document.activeElement))return;item.classList.remove('desktop-open');trigger.setAttribute('aria-expanded','false')},delay)};
 item.addEventListener('pointerenter',openDesktop);
 item.addEventListener('pointerleave',()=>closeDesktop(180));
 panel.addEventListener('pointerenter',openDesktop);
 panel.addEventListener('pointerleave',()=>closeDesktop(120));
 trigger.addEventListener('focus',openDesktop);
 item.addEventListener('focusout',event=>{if(!item.contains(event.relatedTarget))closeDesktop(0)});
});
const collapseMobileNavItems=()=>mainNav?.querySelectorAll('.nav-item.expanded').forEach(item=>{item.classList.remove('expanded');item.querySelector(':scope > a')?.setAttribute('aria-expanded','false');item.querySelector(':scope > .mobile-submenu-toggle')?.setAttribute('aria-expanded','false')});
const setMobileMenuOpen=open=>{mainNav?.classList.toggle('open',open);menuBtn?.setAttribute('aria-expanded',String(open));menuBtn?.setAttribute('aria-label',open?(document.documentElement.lang==='zh'?'关闭菜单':'Close menu'):'Menu');document.body.classList.toggle('mobile-nav-open',open);if(open&&mainNav)mainNav.scrollTop=0;if(!open)collapseMobileNavItems()};
menuBtn?.addEventListener('click',()=>{const open=!mainNav?.classList.contains('open');if(open){setGlobalSearchGuide(false);langMenu?.classList.remove('open');langBtn?.setAttribute('aria-expanded','false')}setMobileMenuOpen(open)});
const closeGlobalHeaderOverlays=()=>{setGlobalSearchGuide(false);clearDesktopNavOpen();langMenu?.classList.remove('open');langBtn?.setAttribute('aria-expanded','false');setMobileMenuOpen(false)};
const searchGuideInner=globalSearchGuide?.querySelector('.home-search-guide__inner');
if(searchGuideInner&&!searchGuideInner.querySelector('.home-search-guide__mobile-search')){
 const mobileSearchForm=document.createElement('form');
 mobileSearchForm.className='home-search-guide__mobile-search';
 mobileSearchForm.setAttribute('role','search');
 mobileSearchForm.innerHTML='<input type="search" autocomplete="off" data-i18n-placeholder="catalog.searchPlaceholder" placeholder="Search name, collection, style or category" /><button type="submit" data-i18n-aria-label="search.button" aria-label="Search"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m21 21-4.35-4.35m2.1-5.15a7.25 7.25 0 1 1-14.5 0 7.25 7.25 0 0 1 14.5 0Z"/></svg></button>';
 const mobileSearchInput=mobileSearchForm.querySelector('input');
 mobileSearchInput.value=new URLSearchParams(location.search).get('q')||'';
 mobileSearchForm.addEventListener('submit',event=>{event.preventDefault();const q=mobileSearchInput.value.trim();if(q)location.href='products.html?q='+encodeURIComponent(q)});
 searchGuideInner.prepend(mobileSearchForm);
}
globalSiteSearch?.addEventListener('focus',()=>setGlobalSearchGuide(true));
globalSearchForm?.addEventListener('click',()=>{if(matchMedia('(max-width: 1114px)').matches){setMobileMenuOpen(false);setGlobalSearchGuide(true)}});
...
if(page==='home'){
 ...
 homeHeader?.addEventListener('mouseleave',()=>{if(matchMedia('(min-width: 1115px) and (hover: hover) and (pointer: fine)').matches)closeHomeHeaderOverlays()});
...
const taxonomyTypeKeys={'Sofas':'type.sofas','Armchairs':'type.armchairs','Coffee Tables':'type.coffeeTables','TV Stands':'type.tvStands','Side Tables':'type.sideTables','Bookcases':'type.bookcases','Ottomans':'type.ottomans','Console Tables':'type.consoleTables','Beds':'type.beds','Nightstands':'type.nightstands','Dressers':'type.dressers','Wardrobes':'type.wardrobes','Dressing Tables':'type.dressingTables','Dressing Chair':'type.dressingChair','Benches':'type.benches','Mirrors':'type.mirrors','Dining Tables':'type.diningTables','Dining Chairs':'type.diningChairs','Sideboards':'type.sideboards','Bar Carts':'type.barCarts','Bar Stools':'type.barStools','Display Cabinets':'type.displayCabinets','Office Desks':'type.officeDesks','Office Chairs':'type.officeChairs','Filing Cabinets':'type.filingCabinets','Credenzas':'type.credenzas','Desk Accessories':'type.deskAccessories','Shoe Cabinets':'type.shoeCabinets','Coat Racks':'type.coatRacks','Umbrella Stands':'type.umbrellaStands','outdoors':'type.outdoors','Accessories':'type.accessories','others':'type.others'};
const taxonomyTypeAliases={'Armchairs':['Accent Chairs'],'TV Stands':['TV Stands & Media Consoles'],'Side Tables':['Side / End Tables'],'Bookcases':['Bookcases & Cabinets'],'Beds':['Bed Frames'],'Dressers':['Dressers & Chests'],'Wardrobes':['Armoires / Wardrobes'],'Display Cabinets':['Bookcases & Cabinets','Cabinets'],'Office Desks':['Desks'],'Filing Cabinets':['Cabinets'],'Credenzas':['Cabinets'],'Shoe Cabinets':['Entryway Cabinets'],'Dressing Tables':['Vanity Table'],'Dressing Chair':['Vanity Stool','Dressing Stool','Vanity Chair'],'Bar Stools':['Bar / Counter Stools']};
const taxonomyTypeMatches=(productType,requestedType)=>!requestedType||productType===requestedType||(taxonomyTypeAliases[requestedType]||[]).includes(productType);
function canonicalLabel(param,val,lang){if(!val)return tx(lang,'nav.all');const roomMap={'Living Room':'room.living','Bedroom':'room.bedroom','Dining Room':'room.dining','Home Office':'room.office','Entryway & Hallway':'room.entryDecor','Others':'room.others','Bathroom':'room.bathroom','Other':'room.other'};const styleMap={'British Manor':'style.traditional','French Chateau 法式城堡':'style.modern','French Chateau':'style.modern','French Château':'style.modern','Italian Renaissance':'style.farmhouse','American Legacy':'style.rustic','Traditional American':'style.traditional','Modern American':'style.modern','Farmhouse':'style.farmhouse','Rustic':'style.rustic','Coastal':'style.coastal','Mid-Century American':'style.midcentury','Other':'nav.other'};const collMap={'Fuyi Collection':'collection.fuyi','Art Collection':'collection.art','RH-Style Collection':'collection.rh','Puffpop sofa':'collection.puffpop','Other':'nav.other'};const typeMap={'Sofa & Sectionals':'type.sofas','Coffee Tables':'type.coffee','Side / End Tables':'type.side','TV Stands & Media Consoles':'type.tv','Accent Chairs':'type.accent','Bookcases & Cabinets':'type.bookcases','Bed Frames':'type.beds','Nightstands':'type.nightstands','Dressers & Chests':'type.dressers','Armoires / Wardrobes':'type.wardrobes','Sofas':'nav.sofa','Ottomans':'nav.sofa.ottoman','Dining Tables':'type.diningTables','Dining Chairs':'type.diningChairs','Sideboards & Buffets':'type.sideboards','Bar Cabinets':'nav.dining.barCabinet','Office Desks':'type.officeDesks','Office Chairs':'type.officeChairs','Bookcases':'type.bookcasesShort','Display Cabinets':'type.displayCabinets','Entryway Cabinets':'type.entrywayCabinets','Shoe Cabinets':'type.shoeCabinets','Console Tables':'type.consoleTables','Benches':'nav.entry.bench',...taxonomyTypeKeys};const maps={room:roomMap,style:styleMap,collection:collMap,type:typeMap};return maps[param]?.[val]?tx(lang,maps[param][val]):val}
...
function initRoomFilterAccordion(){if(page!=='products')return;const params=new URLSearchParams(location.search),currentRoom=params.get('room')||'',currentType=params.get('type')||'',groups=[...document.querySelectorAll('[data-room-filter-group]')];groups.forEach(group=>{const button=group.querySelector('.room-filter__toggle'),symbol=button?.querySelector('span'),panel=group.querySelector('.room-filter__children'),roomLink=group.querySelector('.room-filter__link');if(!button||!symbol||!panel)return;const roomName=roomLink?.textContent.trim()||group.dataset.room||'';const setOpen=open=>{group.classList.toggle('is-open',open);panel.hidden=!open;button.setAttribute('aria-expanded',String(open));symbol.textContent=open?'-':'+';button.setAttribute('aria-label',`${open?'Hide':'Show'} ${roomName} categories`)};setOpen(Boolean(currentRoom&&group.dataset.room===currentRoom));roomLink?.classList.toggle('active',Boolean(currentRoom&&group.dataset.room===currentRoom&&!currentType));panel.querySelectorAll('a').forEach(link=>{const url=new URL(link.href,location.href);link.classList.toggle('active',url.searchParams.get('room')===currentRoom&&url.searchParams.get('type')===currentType)});button.addEventListener('click',()=>setOpen(!group.classList.contains('is-open')))});}
if(page==='products'){
  initRoomFilterAccordion();
 ...
 if(!document.getElementById('catalogPagination')){
  ...
  function applyCatalogFilters(){
   ...
   const roomMatches=(rooms,requested)=>!requested||String(rooms||'').split(',').map(item=>item.trim()).includes(requested);
   products.forEach(card=>{let ok=true;if(initialRoom)ok&&=roomMatches(card.dataset.room,initialRoom);if(collection)ok&&=card.dataset.collection===collection;if(style)ok&&=card.dataset.style===productStyle;if(type)ok&&=taxonomyTypeMatches(card.dataset.type,type);if(q){const hay=[card.dataset.title,card.dataset.room,card.dataset.style,card.dataset.type,card.dataset.collection].join(' ').toLowerCase();ok&&=hay.includes(q)}card.classList.toggle('hidden',!ok);if(ok)visible++});
