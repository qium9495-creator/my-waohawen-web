(function(){
 ...
 function bindFilters(){
  ...
  const source=[...localProducts].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));
  const pageSize=20;
 ...
 if(controls.sort&&params.has('sort'))controls.sort.value=params.get('sort')||'featured';
  ...
  function apply(shouldScroll=false){const currentLang=lang(),room=params.get('room')||'',collection=controls.collection?.value||'',style=controls.style?.value||'',type=controls.type?.value||'',q=(controls.search?.value||'').trim().toLowerCase(),searchActive=Boolean(q);const matches=source.filter(p=>{const productRoom=normalizedRoom(p.room),productStyle=normalizedStyle(p.style),productCollection=p.collection,productType=p.category,hay=[p.sku,p.id,p.name,p.nameZh,displayName(p,currentLang),productRoom,localName(productRoom,roomKeys,currentLang),productCollection,localName(productCollection,collectionKeys,currentLang),productStyle,cleanStyle(p.style,currentLang),productType,formatDimensions(p.dimensions,currentLang)].join(' ').toLowerCase();return searchActive?hay.includes(q):roomMatches(productRoom,room)&&(!collection||productCollection===collection)&&(!style||productStyle===style)&&typeMatches(productType,type)});const ordered=[...matches];if(controls.sort?.value==='az')ordered.sort((a,b)=>(displayName(a,currentLang)||a.sku||'').localeCompare(displayName(b,currentLang)||b.sku||''));if(controls.sort?.value==='price-low')ordered.sort((a,b)=>Number(a.price||0)-Number(b.price||0));if(controls.sort?.value==='price-high')ordered.sort((a,b)=>Number(b.price||0)-Number(a.price||0));const totalPages=Math.max(1,Math.ceil(ordered.length/pageSize));currentPage=Math.min(Math.max(1,currentPage),totalPages);const pageItems=ordered.slice((currentPage-1)*pageSize,currentPage*pageSize);if(grid)grid.innerHTML=pageItems.map(item=>productCardMarkup(item,currentLang)).join('');const countKey=ordered.length===1?'catalog.productCountOne':'catalog.productCount';const count=document.getElementById('resultCount');if(count)count.textContent=t(countKey,currentLang).replace('{count}',ordered.length);document.getElementById('noResults')?.classList.toggle('show',ordered.length===0);renderPagination(totalPages);updatePageUrl();if(shouldScroll)grid?.scrollIntoView({block:'start',behavior:'smooth'})}
