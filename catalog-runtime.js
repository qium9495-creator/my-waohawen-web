(function(){
 ...
 const productNameMap={'沙发单人位':'Single Sofa','沙发双人位':'Two-seat Sofa','沙发三人位':'Three-seat Sofa','沙发四人位':'Four-seat Sofa','茶几':'Coffee Table','餐椅':'Dining Chair','餐桌':'Dining Table','床':'Bed','床头柜':'Nightstand','书桌':'Desk','书柜':'Bookcase','边几':'Side Table','角几':'Side Table','玄关台':'Console Table','玄关柜':'Entryway Cabinet','衣柜':'Wardrobe'};
 ...
 const cleanStyle=(style,currentLang=lang())=>{const value=String(style||'').trim();if(!value)return '';if(currentLang==='zh')return value.includes(' ')?value.split(/\s+/).slice(1).join(' ')||value:value;return value.split(/\s+/)[0]==='British'?'British Manor':value.replace(/[\u4e00-\u9fff].*$/,'').trim()};
 ...
 function renderCatalog(){
  ...
  grid.innerHTML=rows.map(p=>{const name=displayName(p,currentLang),style=cleanStyle(p.style,currentLang);return `<article class="product-card" data-title="${esc(`${p.name||''} ${p.nameZh||''} ${name}`)}" data-style="${esc(style)}" data-room="${esc(p.room)}" data-type="${esc(p.category)}" data-collection="${esc(p.collection)}" data-price="${p.price||0}"><a class="product-image" href="product-detail.html?id=${encodeURIComponent(p.id)}"><img src="${esc(p.images?.[0]||'assets/logo-icon.png')}" alt="${esc(name)}" loading="lazy" decoding="async"></a><a class="product-title-link" href="product-detail.html?id=${encodeURIComponent(p.id)}"><p>${esc(name)}</p></a><small>${esc([p.collection,style,p.room].filter(Boolean).join(' · '))}</small><div class="product-meta"><span class="product-price"><small>${currentLang==='zh'?'参考价':'Reference Price'}</small><strong>${money(p.price)}</strong></span><a href="product-detail.html?id=${encodeURIComponent(p.id)}">${currentLang==='zh'?'查看详情':'View Details'}</a></div></article>`}).join('');
  bindFilters();
 }
 ...
 function renderDetail(){
  ...
  const currentLang=lang(),name=displayName(p,currentLang),style=cleanStyle(p.style,currentLang);document.title=`${name} · WAO HAVEN`;['detailTitle','detailCrumb','detailMeta'].forEach(id=>document.getElementById(id)?.removeAttribute('data-i18n'));const desc=document.querySelector('.detail-description');desc?.removeAttribute('data-i18n');document.getElementById('detailTitle').textContent=name;document.getElementById('detailCrumb').textContent=name;document.getElementById('detailMeta').textContent=[p.collection,style,p.room].filter(Boolean).join(' · ');if(desc)desc.textContent=localized(p,'description',currentLang);
  const values={sku:p.sku,collection:p.collection,material:localized(p,'material',currentLang),dimensions:p.dimensions,finish:localized(p,'finish',currentLang),custom:currentLang==='zh'?'尺寸、木色、面料与包装可定制':'Size, wood color, fabric and packaging can be customized'};Object.entries(values).forEach(([key,value])=>{const node=document.querySelector(`[data-spec="${key}"]`);if(node&&value)node.textContent=value});
