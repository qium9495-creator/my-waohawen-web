(function(){
 ...
 const money=value=>value==null?t('price.inquiry'):`¥${Number(value).toLocaleString('zh-CN')}`;
 const esc=s=>String(s||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const lang=()=>document.documentElement.lang||'en';
 const productNameMap={
  en:{'沙发单人位':'Single Sofa','沙发双人位':'Two-seat Sofa','沙发三人位':'Three-seat Sofa','沙发四人位':'Four-seat Sofa','茶几':'Coffee Table','茶几桌':'Coffee Table','餐椅':'Dining Chair','餐桌':'Dining Table','床':'Bed','床头柜':'Nightstand','书桌':'Desk','书柜':'Bookcase','边几':'Side Table','角几':'Side Table','玄关台':'Console Table','玄关柜':'Entryway Cabinet','衣柜':'Wardrobe'},
  es:{'沙发单人位':'Sofá individual','沙发双人位':'Sofá de dos plazas','沙发三人位':'Sofá de tres plazas','沙发四人位':'Sofá de cuatro plazas','茶几':'Mesa de centro','茶几桌':'Mesa de centro','餐椅':'Silla de comedor','餐桌':'Mesa de comedor','床':'Cama','床头柜':'Mesita de noche','书桌':'Escritorio','书柜':'Librero','边几':'Mesa auxiliar','角几':'Mesa auxiliar','玄关台':'Consola','玄关柜':'Gabinete de entrada','衣柜':'Armario'},
  ru:{'沙发单人位':'Одноместный диван','沙发双人位':'Двухместный диван','沙发三人位':'Трехместный диван','沙发四人位':'Четырехместный диван','茶几':'Журнальный столик','茶几桌':'Журнальный столик','餐椅':'Обеденный стул','餐桌':'Обеденный стол','床':'Кровать','床头柜':'Прикроватная тумба','书桌':'Письменный стол','书柜':'Книжный шкаф','边几':'Приставной столик','角几':'Приставной столик','玄关台':'Консольный стол','玄关柜':'Шкаф для прихожей','衣柜':'Шкаф'},
  de:{'沙发单人位':'Einzelsessel-Sofa','沙发双人位':'Zweisitzer-Sofa','沙发三人位':'Dreisitzer-Sofa','沙发四人位':'Viersitzer-Sofa','茶几':'Couchtisch','茶几桌':'Couchtisch','餐椅':'Esszimmerstuhl','餐桌':'Esstisch','床':'Bett','床头柜':'Nachttisch','书桌':'Schreibtisch','书柜':'Bücherregal','边几':'Beistelltisch','角几':'Beistelltisch','玄关台':'Konsolentisch','玄关柜':'Dielenschrank','衣柜':'Kleiderschrank'},
  fr:{'沙发单人位':'Canapé une place','沙发双人位':'Canapé deux places','沙发三人位':'Canapé trois places','沙发四人位':'Canapé quatre places','茶几':'Table basse','茶几桌':'Table basse','餐椅':'Chaise de salle à manger','餐桌':'Table à manger','床':'Lit','床头柜':'Table de chevet','书桌':'Bureau','书柜':'Bibliothèque','边几':'Table d’appoint','角几':'Table d’appoint','玄关台':'Console','玄关柜':'Meuble d’entrée','衣柜':'Armoire'},
  zh:{},
  ar:{'沙发单人位':'أريكة مفردة','沙发双人位':'أريكة بمقعدين','沙发三人位':'أريكة بثلاثة مقاعد','沙发四人位':'أريكة بأربعة مقاعد','茶几':'طاولة قهوة','茶几桌':'طاولة قهوة','餐椅':'كرسي طعام','餐桌':'طاولة طعام','床':'سرير','床头柜':'كومود جانبي','书桌':'مكتب','书柜':'خزانة كتب','边几':'طاولة جانبية','角几':'طاولة جانبية','玄关台':'طاولة كونسول','玄关柜':'خزانة مدخل','衣柜':'خزانة ملابس'}
 };
 ...
 function renderCatalog(){
  ...
  grid.innerHTML=rows.map(p=>{const name=displayName(p,currentLang),style=cleanStyle(p.style,currentLang),collection=localName(p.collection,collectionKeys,currentLang),room=localName(p.room,roomKeys,currentLang),dimensions=formatDimensions(p.dimensions),imageSrc=preferredProductImage(p.images);return `<article class="product-card" data-title="${esc(`${p.name||''} ${p.nameZh||''} ${name}`)}" data-style="${esc(style)}" data-room="${esc(p.room)}" data-type="${esc(p.category)}" data-collection="${esc(p.collection)}" data-price="${p.price||0}"><a class="product-image" href="product-detail.html?id=${encodeURIComponent(p.id)}"><img src="${esc(imageSrc)}" alt="${esc(name)}" loading="lazy" decoding="async"></a><a class="product-title-link" href="product-detail.html?id=${encodeURIComponent(p.id)}"><p>${esc(name)}</p></a><small>${esc([collection,style,room].filter(Boolean).join(' · '))}</small>${dimensions?`<div class="product-dimensions"><span>${esc(t('product.dimensions',currentLang))}</span><strong>${esc(dimensions)}</strong></div>`:''}<div class="product-meta"><span class="product-price"><small>${esc(t('price.reference',currentLang))}</small><strong>${money(p.price)}</strong></span><a href="product-detail.html?id=${encodeURIComponent(p.id)}">${esc(t('product.details',currentLang))}</a></div></article>`}).join('');
  bindFilters();
 }
 ...
 function renderDetail(){
  ...
  const values={sku:p.sku,collection,dimensions:formatDimensions(p.dimensions),finish:localized(p,'finish',currentLang),custom:t('product.customValue',currentLang)};Object.entries(values).forEach(([key,value])=>{const node=document.querySelector(`[data-spec="${key}"]`);if(node&&value)node.textContent=value});
